...
%\frametitle{Bankkonto med ränta}
%\begin{itemize}
%  \item Attributet \code{interestRate} anger räntesatsen i procent för kontot.
%  \item Den ackumulerade räntan (attributet \code{interest}) ökas genom anrop av metoden \code{computeInterest} varje gång som saldot förändras genom insättning eller uttag.
%  \item Attributet \code{lastInterestDay} anger numret på den dag (med början på nummer 1 för den 1 januari) då räntan senast beräknades.
%  \item Vi förutsätter att vi har tillgång till en variabel \code{Date.today} som ger numret på aktuell dag.
%  \item I slutet av ett år läggs den ackumulerade räntan till saldot när någon ''utifrån'' utför operationen \code{newYearActions}.
%\end{itemize}
%\end{frame} 
%
%\begin{frame}[fragile=singleslide]
%\frametitle{Implementering av \code{BankAccountWithInterest}, 1}
%\begin{Code}
...
%
%\begin{frame}[fragile=singleslide]
%\frametitle{Implementering av \code{BankAccountWithInterest}, 2}
%\begin{Code}
